...
| `lab-sample.yaml` | Complete working example (canonical flat format) |
...
Validates against lab-schema.json
  (worker also validates on startup)
  ↓
Worker seeds metadata → Firestore
                                 Reads lab.yaml
                                 Resolves environment (string ref)
...
## The Canonical (Flat) Format

One lab per directory — `lab.yaml` + `instructions.md`. Environment config is shared — referenced by name from `environments/{name}.yaml`.
...
| `tasks` | no | object[] | Ordered list of tasks. Omit for a skeleton lab (warns, not fails). |
...
| `type` | yes | `multiple_choice`, `terminal_action`, `file_check`, or `port_check` |
...
| Type | Required validation fields |
|------|----------------------------|
| `multiple_choice` | `expected_output` (static options) **or** `command` (dynamic options) |
| `terminal_action` | `command` + `expected_output` |
| `port_check` | `command` (currently) or `port` / `path` |
| `file_check` | `path` + `contains` |

### 4. Validation

Controls how the backend checks answers (inside the container).

| Field | Required | Description |
|-------|----------|-------------|
| `command` | varies | Shell command to run inside the container. Required for `terminal_action` and `multiple_choice` (dynamic). |
| `expected_output` | varies | The expected result. Required for `exact`, `contains`, `regex`, `line_count` match types. |
...
| `path` | no | File path (for `file_check` / `port_check`). |
| `contains` | no | String the file must contain (for `file_check`). |
| `port` | no | Port number (for `port_check`). |
...
For a full end-to-end walkthrough (validation, seeding, rendering), see `docs/CONTENT-PIPELINE.md`.
