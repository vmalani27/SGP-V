    ...
    <div className="space-y-4">
      ...
      <div className="p-3 rounded-lg bg-gray-800/50 border border-gray-700/50 space-y-2">
        <p className="text-xs text-muted">
          <span className="text-gray-400">Service check:</span> Port <code className="text-amber-400 bg-amber-500/10 px-1 rounded">{port}</code>
          {path !== '/' && <> at <code className="text-amber-400 bg-amber-500/10 px-1 rounded">{path}</code></>}
        </p>
        <p className="text-xs text-muted">Use the terminal on the right to start your service, then validate.</p>
      </div>
      ...
      {status !== 'correct' && (
        ...
        >
          {validating ? 'Checking port...' : 'Check Service'}
        </button>
      )}
